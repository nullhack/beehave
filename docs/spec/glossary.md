...
**Definition:** A Python file generated by `beehave generate` for an orphan scenario, containing imports (hypothesis strategies, beehave decorators), module-level strategy variables for all `<placeholders>` (defaulting to `st.integers()`), step decorators matching .feature steps, @Example decorators from .feature Examples table, and a function named `test_<scenario_title_snake_case>_<id>` with `@pytest.mark.skip(reason="not yet implemented")` decorator and `raise NotImplementedError` body, immediately importable and collectable by pytest as SKIPPED.

**Aliases:** skeleton (deprecated)

**Example:** `beehave generate` produces a test stub with `@pytest.mark.skip(reason="not yet implemented")` / `def test_withdrawal_insufficient_funds_a1b2c3d4(): raise NotImplementedError` ready for pytest collection as SKIPPED.
...
**Definition:** The default human-readable output format of `beehave generate`, `beehave fix`, and `beehave clean` commands, showing one line per action (e.g., "Created tests/features/foo/default_test.py for @a1b2c3d4"). Contrasted with JSON mode (`--json`) for programmatic consumption.

**Aliases:** text output, default mode

**Example:** `beehave generate` without `--json` flag produces "Created tests/features/balance/default_test.py for @a1b2c3d4".

**Source:** 2026-05-11

## Stub Body Convention

**Definition:** The project's TDD convention for unimplemented test functions: `@pytest.mark.skip(reason="not yet implemented")` decorator followed by `raise NotImplementedError` as the function body. Generated stubs must follow this convention so they appear as SKIPPED (not PASSED) in pytest collection.

**Aliases:** skip-marker convention

**Example:** A generated stub `@pytest.mark.skip(reason="not yet implemented")` / `def test_withdrawal_a1b2c3d4(): raise NotImplementedError` appears as SKIPPED in pytest output.

**Source:** 2026-05-11

## Strategy Fallback

**Definition:** When `_resolve_placeholder()` finds no module-level strategy variable and no @Example type inference for a `<placeholder>`, it defaults to `st.integers()`. This produces any integer, which may violate domain constraints. A `UserWarning` is emitted identifying the placeholder name so developers can define an explicit strategy.

**Aliases:** integer fallback, default strategy

**Example:** A test using `<quantity>` with no `quantity_strategy` defined triggers a UserWarning: "placeholder 'quantity' using st.integers() fallback — define quantity_strategy for domain-specific values".
